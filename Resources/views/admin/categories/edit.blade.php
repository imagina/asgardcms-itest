...
@section('content-header')
    <h1>
        {{ trans('itest::categories.title.edit category') }}
    </h1>
    <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-xs-12 col-md-9">
            <div class="row">
                            ...
                            <div class="tab-content">
                                ...
                                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                    ...
                                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                        @include('itest::admin.categories.partials.edit-fields', ['lang' => $locale])
                                    </div>
                                @endforeach
                ...
                @if (config('asgard.itest.config.fields.category.partials.normal.edit') && config('asgard.itest.config.fields.category.partials.normal.edit') !== [])
                                ...
                                @foreach($status as $index=>$item)
                                    <label class="radio" for="{{$item}}">
                                        <input type="radio" id="status" name="status"
                                               value="{{$index}}" {{old('status',$category->status) == $index ? 'checked' : '' }}>
                                        {{$item}}
                                    </label>
                                @endforeach
                            </div>
