...
@section('content-header')
    <h1>
        {{ trans('itest::questions.title.edit question') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.itest.question.index') }}">{{ trans('itest::questions.title.questions') }}</a></li>
        <li class="active">{{ trans('itest::questions.title.edit question') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.itest.question.update', $question->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-xs-12 col-md-9">
            <div class="row">
                <div class="col-xs-12">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('itest::admin.questions.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
                ...
                @if (config('asgard.itest.config.fields.question.partials.normal.edit') && config('asgard.itest.config.fields.question.partials.normal.edit') !== [])
                ...
                <div class="col-xs-12 ">
                    <div class="box box-primary">
                        ...
                        <div class="box-body ">
                            @foreach (config('asgard.itest.config.fields.question.partials.normal.edit') as $partial)
                            @include($partial)
                            @endforeach
                        ...
                        <div class="box-body ">
                            <div class="box-footer">
                                ...
                                <a class="btn btn-danger pull-right btn-flat"
                                   href="{{ route('admin.itest.question.index')}}">
                            ...
                            <div class="tab-content">
                                <div class="form-group">
                                    <div class='input-group date' id='created'>
                                        <input type='text' name="created_at" id="created_at" class="form-control" value="{{$question->created_at}}"/>
                                ...
                                @foreach($status as $index=>$item)
                                    <label class="radio" for="{{$item}}">
                                        <input type="radio" id="status" name="status"
                                               value="{{$index}}" {{old('status',$question->status) == $index ? 'checked' : '' }}>
                                        {{$item}}
                                    </label>
                                @endforeach
                            </div>
                        ...
                        <div class="box-body">
                            <select name="user_id" id="user" class="form-control">
                                @foreach ($users as $user)
                                    <option value="{{$user->id }}" {{$user->id == old('user_id',$question->user_id) ? 'selected' : ''}}>{{$user->present()->fullname()}}
                                        - ({{$user->email}})
                                    </option>
                                @endforeach
                            </select><br>
